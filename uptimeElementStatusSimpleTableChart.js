			...
			uptime_api.getElementStatus(entityId, function(elementStatus) {
				...
				if (deps.length > 0) {
					for ( var i = 0; i < deps.length; i++) {
						...
						divboxes += alink + '<div class="topoBox color-text-' + deps[i].status.toUpperCase() + '">'
								+ deps[i].name + "</div></a>";
					}
				}
				...
				$("#headerTable").append(
						"<tr><th id='bigTableHeading'><a href='/main.php?section=Profile&subsection=&id=" + elementStatus.id
								+ "&name=" + elementStatus.name + "' target='_top'>" + entityName.toUpperCase()
				...
				$.each(elementStatus.monitorStatus, function() {
					...
					if (this.isHidden == false) {
						var alink = "<a href='/main.php?section=Profile&subsection=&id=" + elementStatus.id + "&name="
								+ elementStatus.name + "&dlsection=s_status' target='_top'>"; // <a
						// href=...
						// tag
						// for
						// the
						...
						var monitorStatus = "<td class='color-text-" + this.status.toUpperCase() + "'>" + this.status + "</td>";
						...
						$("#statusTable").append(
								"<tr>" + monitorName + monitorStatus + monitorLastCheck + monitorLastTransition + monitorMessage
										+ monitorIsAckd + monitorAckMessage + "</tr>");
					}
				});
