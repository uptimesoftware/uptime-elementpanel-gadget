			...
			uptime_api
					.getElementStatus(entityId,
							function(elementStatus) {
								...
								$(chartDivId).append(
										"<table id='statusTable' style='width:100%; border:0px;'><tbody></tbody></table>");
								$(chartDivId).append(
										"<table id='topologyTable' style='width:100%; border:0px;'><tbody></tbody></table>");
								...
								var statusIcon = UPTIME_GADGET_BASE + "/images/status-" + elementStatus.status.toLowerCase()
										+ ".png";
								...
								if (deps.length > 0) {
									for ( var i = 0; i < deps.length; i++) {
										var alink = "<a href='/main.php?section=Profile&subsection=&id=" + deps[i].id + "&name="
												+ deps[i].name + "&dlsection=s_status' target='_top'>";
										divboxes += alink + "<div id='topBox" + deps[i].status.toUpperCase() + "'>"
												+ deps[i].name + "</div></a>";
									}
								}
								$("#topologyTable").append(
										"<tr><th colspan='2' style='text-align: left;'><div>Topological Dependencies:</div>"
												+ divboxes + "</th></tr>");
								...
								statusIcon = "<img src='" + statusIcon + "' width='" + statusIconSize + "' height='"
										+ statusIconSize + "' border='0px' />";
								$("#headerTable").append(
										"<tr><th id='bigTableHeading'><a href='/main.php?section=Profile&subsection=&id="
												+ elementStatus.id + "&name=" + elementStatus.name + "' target='_top'>"
												+ elementIcon + entityName.toUpperCase() + "<br/><small><small>"
												+ elementStatus.status + " for " + stateLength + "</small></small>"
								...
								$.each(elementStatus.monitorStatus, function() {
									...
									if (this.isHidden == false) {
										var alink = "<a href='/main.php?section=Profile&subsection=&id=" + elementStatus.id
												+ "&name=" + elementStatus.name + "&dlsection=s_status' target='_top'>"; // <a
										// href=...
										// tag
										// for
										// the
										...
										var monitorStatus = "<td class='" + this.status.toUpperCase() + "'>" + this.status
												+ "</td>";
										...
										if (doLastTransitionTime) {
											...
											// "</td>";
											monitorLastTransition = "<td>"
													+ DateDiff.getDifferenceInEnglish(currentDateTime, Date
															.parse(this.lastTransitionTime)) + "</td>";
										}
										if (doMessage) {
								...
								$(lastRefreshBarDivId).html(
										"<small>Last refreshed: " + checkDateTimeTwoDigits(dt.getMonth() + 1) + "/"
												...
												+ checkDateTimeTwoDigits(dt.getHours()) + ":"
												+ checkDateTimeTwoDigits(dt.getMinutes()) + ":"
